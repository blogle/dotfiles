...
let
  ...
  st = (pkgs.st.override {
    ...
    # nixpkgs' st postPatch concatenates these commands without a separator.
    postPatch = builtins.replaceStrings [ "config.def.hsubstituteInPlace" ]
      [ "config.def.h\nsubstituteInPlace" ] old.postPatch;
  });
...
in
{
  ...
  # Replace the binary generated by older Home Manager generations.
