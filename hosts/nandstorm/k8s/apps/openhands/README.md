...
- `/api/v1`, `/api/keys/current`, `/api/organizations`: sandbox-server at
  `openhands-control.thejeffer.net` (unauthenticated in this pass)
- `/sandbox/<runtime-id>`: runtime adapter ingress and remote Agent Server

The Cloud backend uses the separate control hostname because the browser must
call the Cloud API directly. Tinyauth remains on the Canvas origin; the
control hostname is intentionally unauthenticated in this pass. The
deployment has one remote sandbox profile and zero warm replicas. It does not
provide per-conversation image selection or OIDC application integration.
